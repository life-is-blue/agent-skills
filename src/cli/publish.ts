...
async function uploadArticleImage(imagePath: string): Promise<string> {
  ...
  form.append(
    "media",
    new Blob([prepared.data], { type: prepared.mime }),
    prepared.filename,
  );
...
async function prepareImage(
  ...
  if (ext === ".webp" || ext === ".gif") {
    ...
    if (jpgData.length > maxBytes) {
      cleanupTemp(jpgPath);
      throw new Error(
        `转换后图片 ${basename(absPath)} 仍超过大小限制 (${fmtBytes(
          jpgData.length,
        )} > ${fmtBytes(maxBytes)})，即使 quality=40`,
      );
    }
  ...
  if (fileData.length > maxBytes) {
    throw new Error(
      `图片 ${basename(absPath)} 超过大小限制 (${fmtBytes(
        fileData.length,
      )} > ${fmtBytes(maxBytes)})`,
    );
  }
  ...
  if (!mime) {
    throw new Error(`不支持的图片格式: ${ext}。微信 uploadimg 仅支持 jpg/png`);
  }
...
async function createDraft(article: WeChatArticle): Promise<string> {
  ...
  if (contentBytes > LIMITS.CONTENT_BYTES) {
    throw new Error(
      `文章内容超过微信限制: ${fmtBytes(contentBytes)} (上限 ${fmtBytes(
...
type MutableCliOpts = Partial<Omit<CliOpts, "theme" | "dryRun">> &
  Pick<CliOpts, "theme" | "dryRun">;

function parseArgs(): CliOpts {
  const args = process.argv.slice(2);
  const opts: MutableCliOpts = {
    theme: DEFAULT_THEME,
    dryRun: false,
  };
  ...
  if (!opts.file) {
    console.error(
      `用法: bun run publish <markdown-file> [options]
...
async function main() {
  ...
  const sourceUrl =
    opts.sourceUrl || fm["source_url"] || (fm["source"] as any)?.url || "";
  ...
  if (html.length < rawHtml.length) {
    console.log(
      `  ✓ HTML 转换完成 (${html.length} chars, 去除注释节省 ${
        rawHtml.length - html.length
      } chars)`,
    );
  } else {
