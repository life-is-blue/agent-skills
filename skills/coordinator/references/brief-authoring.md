# 任务书法（brief authoring）· 你出想法，我出任务书

Tier 2 工具：方向已清晰、单轮可交付的复杂任务，冻结成一份执行 agent 能独立跑完的任务书。

三个角色：**领导**（用户）出想法、拍板；**管理者**（你，协调者）调研、写书、验收；**执行者**（目标模式里干活的 agent）拿书独立跑完。任务书必须自包含，但不能覆盖更高优先级指令或既有权限边界。执行者发现前提错误时提供证据，暂停依赖该前提的步骤，由协调者修正合同；不要把常规调查转交用户。
...
**2 提问，一轮 ≤5 个。**只问查不到且会改变任务书的：方向取舍、验收裁量、风险偏好、时间盒；每个给 2–4 个选项加推荐。拆多份并行需明确授权；已有授权覆盖时不重复询问。授权范围内的可逆细节采用安全默认值并披露；披露不要求确认，也不替代批准。未回复不构成实质方向、风险、费用或范围选择的授权；只暂停依赖答案的步骤。

**3 写书**（规则见下节）。

**4 执行或交付。**用户要求实施时，在已有执行、provider 和预算授权内派发、监控、修复并验收，不停在任务书阶段。仅要求任务书，或确实无法访问执行环境时，才按以下格式交付转交材料：
...
**5 验收，是管理者的活。**明卷（验收命令）在书里，目标模式自己盯；暗卷——2–3 条执行者看不见的抽查——自留在会话侧 scratchpad，不进书。自行派发的任务完成后继续验收；只有转交执行的任务才等待结果回来。亲自复跑明卷＋暗卷，给简短人话报告：过没过／干成了什么／遗留问题／下一步。执行者不能自己批卷，领导全程零命令。摸不到执行环境时才退化为「验收官提示词」，让领导转贴给一个没参与执行的 agent。

## 写书规则

任务书六节的结构规格与探索型改法见 [brief-anatomy.md](brief-anatomy.md)——它规定每节写什么，措辞和详略由你贴合任务判断。
...
**接口先行**：复杂多模块任务先冻结接口声明合同——模块边界、函数签名、数据形状——再派实现轮。接口轮与实现轮分开冻结；接口没定，不准各模块各猜各的。

**法与情报分家**：「不许」是法，违反即不合格，每条溯源到一次实测或一次领导裁决；「建议」是情报，执行者有更好的路可以走、在 `PROGRESS.md` 记一句为什么。把情报写成法，是替执行者做它临场更懂的决定。

**交代现场**：修复轮／微修复的任务书，引用的每个文件必须写明此刻的真实状态（在不在、被谁挪到了哪）。协调者在两场派发之间做过的事（归档旧产物、清理现场、挪走坏信封）执行者看不见——不交代，它只能瞎猜，或者更糟：凭空重建一个看似合理的现场。
...
4. **一条道走到黑**：任务 0 核验前提；数字不符先判断是否影响安全或验收，仅暂停依赖该前提的步骤。同一验收连败 3 次结束当前尝试、记录未完成项并交协调者处理，不得悄悄略过。结果比基线差就回滚；失败报告可满足本轮报告契约，不能算原目标完成。遵守已声明的运行重试、时间和费用上限
...
全书大白话，领导一遍读懂：标题直陈功能；术语首次出现括号给半句人话；内部概念名（如「暗卷」「探索型」）不出现在书里。默认零玩笑——每句话要么改变执行者行为，要么删掉；领导点名要梗才读 [brief-style.md](brief-style.md)。
...
4. 前提错误有证据与协调者处理路径？大白话？零多余玩笑？
5. 实施请求已接入执行与验收，还是确需转交？转交材料完整？暗卷自留没进书？
6. 多 agent：全局段齐、地界不重叠、取证不互相作废？

## 升级判据

任务书发出后如果需要多轮迭代、实现与验收必须分离、或要跨会话续跑，回到协调者分级路由进 Tier 3——本文件的「明卷／暗卷」在 Tier 3 协议里叫 open/withheld checks，是同一件事的两种说法。
