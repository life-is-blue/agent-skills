...
async function main() {
  ...
  if (args.length < 1) {
    console.error("Usage: bun run pdf2md <input.pdf> [password]");
    process.exit(1);
  }
...
async function extractPdfText(
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    for (const item of content.items as any[]) {
      ...
      if (isDigit && (y < CONFIG.pageNumYBottom || y > CONFIG.pageNumYTop))
        continue;
...
function groupTextIntoLines(items: TextItem[]): VisualLine[] {
  ...
  for (let i = 1; i < items.length; i++) {
    ...
    if (
      ...
      Math.abs(item.y - current.y) < CONFIG.lineYTolerance
    ) {
...
function assembleMarkdown(lines: VisualLine[]): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (buffer) {
      ...
      const prevIsShort =
        prev.maxX - prev.minX < maxLineW * CONFIG.shortLineRatio;
